...
# 安装：crontab -e 添加 */5 * * * * /opt/mootdx-server/health-check.sh

set -euo pipefail

# ============ 配置 ============
SERVICE_NAME="mootdx"
APP_DIR="/opt/mootdx-server"
...
check_service() {
    local response
    response=$(curl -s -o /dev/null -w "%{http_code}" --max-time 5 http://localhost:8888/health 2>/dev/null || echo "000")
