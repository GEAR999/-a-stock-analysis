...
export function AnalysisPanel() {
  ...
  return (
    <div className="rounded border border-border bg-[var(--bg-panel)] p-3">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-medium text-[var(--text-primary)]">分析理论开关</h3>
        <TooltipProvider>
          <Tooltip>
            <TooltipTrigger asChild>
              <Info className="w-3.5 h-3.5 text-[var(--text-secondary)] cursor-help" />
            </TooltipTrigger>
            <TooltipContent side="right" className="max-w-[250px] bg-[var(--bg-primary)] border-border">
              <p className="text-xs text-[var(--text-primary)]">开启后会在下方显示该理论的独立分析卡片，包含走势研判、多路径推演和操作建议</p>
            </TooltipContent>
          </Tooltip>
        </TooltipProvider>
      </div>
      <div className="space-y-2">
        <label className="flex items-center gap-2 cursor-pointer group">
          <input
            ...
            className="w-3.5 h-3.5 rounded border-[var(--border-default)] bg-[var(--bg-primary)] text-purple-500 focus:ring-purple-500/20"
          />
          <div className="w-2 h-2 rounded-full bg-purple-500" />
          <span className="text-xs text-[var(--text-secondary)] group-hover:text-[var(--text-primary)]">缠论分析</span>
        </label>
        <label className="flex items-center gap-2 cursor-pointer group">
          <input
            type="checkbox"
            ...
            className="w-3.5 h-3.5 rounded border-[var(--border-default)] bg-[var(--bg-primary)] text-[var(--accent-blue)] focus:ring-blue-500/20"
          />
          <div className="w-2 h-2 rounded-full bg-blue-500" />
          <span className="text-xs text-[var(--text-secondary)] group-hover:text-[var(--text-primary)]">波浪理论</span>
        </label>
        {/* 波浪灵敏度 */}
        {analysisSettings.wave && (
          <div className="ml-5 pl-2 border-l border-[var(--accent-blue)]/20">
            <div className="flex items-center gap-1 mb-1">
              <span className="text-[10px] text-[var(--text-secondary)]">灵敏度</span>
            </div>
                ...
                <button
                  ...
                  className={`px-2 py-0.5 text-[10px] rounded transition-colors ${
                    analysisSettings.waveSensitivity === value
                      ? 'bg-blue-500/20 text-blue-300 border border-[var(--accent-blue)]/30'
        ...
        <label className="flex items-center gap-2 cursor-pointer group">
          <input
            ...
            className="w-3.5 h-3.5 rounded border-[var(--border-default)] bg-[var(--bg-primary)] text-emerald-500 focus:ring-emerald-500/20"
          />
          <div className="w-2 h-2 rounded-full bg-emerald-500" />
          <span className="text-xs text-[var(--text-secondary)] group-hover:text-[var(--text-primary)]">技术指标</span>
        </label>
        ...
        <div className="pt-2 mt-2 border-t border-[var(--border-default)]">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-[var(--text-secondary)]">自定义策略</span>
          ...
          {!showCustomStrategy && customStrategies.length === 0 && (
            <button
              ...
              className="mt-2 w-full flex items-center justify-center gap-1 text-[10px] text-[var(--text-secondary)] hover:text-amber-500 border border-dashed border-[var(--border-default)] hover:border-amber-500/30 rounded py-1.5 transition-colors"
            >
