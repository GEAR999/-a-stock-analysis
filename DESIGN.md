...
## ML 预测面板专用规范
- 统计指标栏：水平 flex 排列，`gap-x-8` 分隔，文字居中，每项 `min-w-[80px]`
- 预测卡片：2行×4列网格，`gap-3`，卡片内方向标签竖排在上，概率横排在下
- 置信度：高置信度(>0.5)用青绿(#34d399)高亮，低置信度用灰蓝(#64748b)淡化
- 特征重要性：水平条形图，红/绿双色，左侧显示特征名，右侧显示重要性数值
- 混淆矩阵：四格 2×2 网格，TP/TN 用绿色系，FP/FN 用红色系，数字居中大号显示
- 预测历史：紧凑列表，每行显示日期、指数、预测方向（红/绿箭头）、概率条、实际结果标记
- 概率条：双向条形（红涨绿跌），中间分界线，宽度表示概率大小
