...
│   │   └── utils.ts           # 回测公共工具函数
│   ├── strategy/
│   │   ├── StrategyLibrary.tsx # 策略库页面 (分类/搜索/排序/管理/编辑器入口)
│   │   ├── StrategyEditor.tsx  # 策略编辑器 (信号/仓位/风控/成本四Tab, 新建/编辑/只读模式)
│   │   ├── StrategyCard.tsx   # 策略卡片组件 (名称/描述/使用次数/操作/点击查看)
...
- **策略库页面**：StrategyLibrary.tsx + StrategyCard.tsx
  ...
  - 点击卡片查看策略详情（只读模式）
  - "新建策略"按钮创建自定义策略
- **策略编辑器**：StrategyEditor.tsx
  - 四个Tab：信号配置 / 仓位控制 / 风控管理 / 成本设置
  - 三种模式：新建（可编辑所有字段）、编辑（可修改自定义/AI策略）、只读（查看内置策略）
  - 信号配置：买卖信号多选、AND/OR逻辑、最小匹配数
  - 仓位控制：总仓位上限、单股上限、现金储备、建仓比例、加仓设置
  - 风控管理：止损/止盈/移动止损/最长持仓天数
  - 成本设置：手续费率/滑点率
...
#### 1. 策略库升级为完整策略定义
- **数据结构**：`StrategyDefinition` 扩展为包含信号 + 仓位 + 风控 + 成本的完整策略
- **新增类型**：
  - `PositionControl`：总仓位上限、单股上限、现金储备、加仓控制
  - `RiskControl`：止损、止盈、移动止损、最长持仓天数
  - `FullStrategyConfig`：整合所有配置的完整策略配置
- **内置策略**：16 个内置策略已升级为完整配置（含默认仓位和风控参数）
- **旧策略迁移**：`migrateLegacyStrategy()` 自动将旧策略转换为新结构
- **文件**：`src/lib/strategy-library.ts`

#### 2. 回测引擎支持仓位控制和风控
- **新增函数**：`runBacktestFull(kline, config)` - 完整版回测
- **仓位控制**：
  - 总仓位上限检查（如最多 80% 仓位）
  - 单股仓位上限（如单只最多 30%）
  - 最低现金储备（如至少留 10% 现金）
...
- **新增结果字段**：
  - `positionLogs`：仓位变化日志
  - `riskEvents`：风控触发事件（止损/止盈/移动止损）
- **文件**：`src/lib/backtest-engine.ts`

#### 3. 历史回测添加 AI 分析功能
- **交互**：点击交易记录时自动调用 DeepSeek AI 分析
- **分析维度**：信号有效性、指标配合、风险评估、改进建议
- **数据基础**：基于交易时刻的 MACD/KDJ/RSI/BOLL 指标快照
- **UI 展示**：分析结果显示在交易详情下方，带加载状态
- **接口**：接入现有 `/api/ai/chat` 接口
- **文件**：`src/components/backtest/HistoryBacktestPanel.tsx`

#### 4. K 线图底部文本栏
- **功能**：悬停买卖点标记时显示交易依据
- **交互**：悬停显示，离开保留，新悬停覆盖
- **内容**：方向、日期、策略、价格、数量、详细依据
- **文件**：`src/components/backtest/BacktestChart.tsx`

### 待完成功能

| 优先级 | 任务 | 说明 | 相关文件 |
|--------|------|------|----------|
| **中** | 交易列表交互 | 解决 K 线图悬停/点击与 dataZoom 冲突的问题（当前已用悬停保留方案） | `BacktestChart.tsx` |
| **低** | 箭头指引 | 从文本栏拉箭头指向图表标记（可选，用户已搁置） | `BacktestChart.tsx` |

### 已完成功能（最新）

#### 策略编辑器 UI
- **StrategyEditor.tsx**：完整的策略编辑器组件，支持新建/编辑/只读三种模式
- 四个Tab分组：信号配置（买卖信号多选+AND/OR逻辑+最小匹配数）、仓位控制（总仓位/单股/现金储备/建仓/加仓）、风控管理（止损/止盈/移动止损/最长持仓）、成本设置（手续费/滑点）
- 集成到策略库页面：点击"新建策略"创建自定义策略，点击卡片查看/编辑策略
- 内置策略只读，自定义和AI生成策略可编辑

#### 回测面板简化
- **HistoryBacktestPanel.tsx**：从多策略多选+分散配置改为单一策略选择
- 策略选择器：下拉列表展示策略库所有策略，显示分类标签和关键参数
- 策略详情面板：选中后展示完整配置（信号/仓位/止损/止盈/手续费/滑点）
- 回测引擎使用 `runBacktestFull` 读取策略的完整配置（position/risk/cost）
- 移除旧的 STRATEGY_OPTIONS 多选和独立的 commission/slippage/positionSize 输入

### 已知问题

1. **K 线图交互冲突**：ECharts 组件层的任何事件（click/mouseover/mouseout）都可能与 dataZoom 冲突
   - 当前方案：使用悬停保留（离开标记后内容不消失），完全移除图表事件
   - 备选方案：交易列表选择（完全不在图表上绑定事件）

2. **策略编辑 UI 未改造**：当前策略库只显示策略列表，缺少完整的策略编辑界面
   - 需要添加：Tab 分组（信号/仓位/风控）、参数配置表单

3. **回测面板配置分散**：当前回测面板仍有独立的策略多选、仓位比例、手续费等配置
   - 需要改为：单一策略选择，所有参数从策略中读取
