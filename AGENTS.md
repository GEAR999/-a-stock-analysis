...
src/
├── app/
│   ├── api/stock/route.ts    # 统一股票API (search/quote/kline/sentiment/stock_sentiment/comprehensive_sentiment)
...
- `GET /api/stock?action=comprehensive_sentiment&code={code}&sector={sector}` - 综合情绪评估（大盘/板块/个股三维度）
